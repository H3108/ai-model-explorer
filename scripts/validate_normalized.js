...
const variantsExtra = (() => { try { return read('model_variants_extra.json') } catch (e) { return [] } })()
...
const variantIds = new Set([...variants, ...variantsExtra].map((v) => v.id))
...
console.log(`[variants] ${variants.length} 个（主）+ ${variantsExtra.length} 个（extra 增量，仅纳入跨引用解析）`)
