...
├── api_endpoints.json   # Base URL、兼容格式、认证方式
...
当前工作区没有连接官方价格或能力数据源，因此无法确认的值均为 `unknown`，没有用估算值填充。`source` 和 `lastVerified` 字段用于未来官方数据更新；页面会明确提示 unknown，不会将未知价格用于“最便宜”结论。
...
├── app.js
├── data/
│   └── Registry JSON files  # 厂商、模型、价格、能力、端点、推荐
...
1. 保留五个 JSON 作为静态兜底数据。
2. 将 `loadData()` 替换为统一数据适配器。
3. 将 `score()` 替换为 Agent 的自然语言意图识别和多目标排序接口。
