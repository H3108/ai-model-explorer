...
| 当前完成度 | **功能约 90%，数据完整度约 78%，工程质量约 55%** |
| 风险等级 | **中**（无 P0；有 3 个 P1，其中 1 个属对外事实性错误） |
...
**任务**：修复 `app.js:724` `priceBlock()` 的货币符号硬编码——将 `¥` 换成既有的 `cur(v.currency)`。

**原因**：这是本次审计中唯一的**对外事实性错误**。48 个美元计价模型（含 GPT-5 全系）在详情页「计费方式」区块被标为人民币，而同一页面上方「关键参数」卡片显示的是美元——同页自相矛盾。用户依据此页做成本测算会产生约 7 倍偏差。修复只需改 1 行，复用项目内已有的 `cur()` 助手，零新增依赖、零回归风险。

**预计收益**：10 分钟内消除 48 个型号页面的价格误导与自相矛盾，恢复站点在「价格准确性」上的可信度——这正是本项目的核心价值主张。
